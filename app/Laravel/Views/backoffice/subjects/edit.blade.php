                    ...
                    <div class="panel panel-default panel-border-color panel-border-color-primary">
                        ...
                        <div class="panel-body">
                            <form method="POST" action="" enctype="multipart/form-data">
                                ...
                                <div class="form-group {{$errors->first('teacher_id') ? 'has-error' : NULL}} xs-pt-10">
                                    <label class="control-label">Teacher</label> 
                                    {{Form::select('teacher_id',$teachers,old('teacher_id',$subject->teacher_id),['class' => "form-control select input-sm"])}}
                                    @if($errors->first('teacher_id'))
                                    <span class="help-block">{!!$errors->first('teacher_id')!!}</span>
                                    @endif
                                </div>
                                <div class="form-group {{$errors->first('school_year') ? 'has-error' : NULL}} xs-pt-10">
                                    <label class="control-label">School Year</label> 
                                    <input class="form-control input-sm" value="{{old('school_year',$subject->school_year)}}" placeholder="Enter School Year" type="text" name="school_year">
                                    @if($errors->first('school_year'))
                                    <span class="help-block">{!!$errors->first('school_year')!!}</span>
                                    @endif
                                </div>

                                <div class="form-group {{$errors->first('subject_title') ? 'has-error' : NULL}} xs-pt-10">
                                    <label class="control-label">Subject Title</label> 
                                    <input class="form-control input-sm" value="{{old('subject_title',$subject->subject_title)}}" placeholder="Enter Subject Title" type="text" name="subject_title">
                                    @if($errors->first('subject_title'))
                                    <span class="help-block">{!!$errors->first('subject_title')!!}</span>
                                    @endif
                                </div>
