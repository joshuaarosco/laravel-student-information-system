                        ...
                        <div class="panel-body">
                            <form method="POST" action="" enctype="multipart/form-data">
                                ...
                                <div class="form-group {{$errors->first('school_year') ? 'has-error' : NULL}} xs-pt-10">
                                    ...
                                    <input class="form-control input-sm" value="{{old('school_year',$sections->school_year)}}" placeholder="Enter School Year" type="text" name="school_year">
                                    @if($errors->first('school_year'))
                                    <span class="help-block">{!!$errors->first('school_year')!!}</span>
                                    @endif
                                </div>

                                <div class="form-group {{$errors->first('section_name') ? 'has-error' : NULL}} xs-pt-10">
                                    <label class="control-label">Section Name</label> 
                                    <input class="form-control input-sm" value="{{old('section_name',$sections->section_name)}}" placeholder="Enter Section Name" type="text" name="section_name">
                                    @if($errors->first('section_name'))
                                    <span class="help-block">{!!$errors->first('section_name')!!}</span>
                                    @endif
                                </div>

                                <div class="form-group {{$errors->first('adviser_id') ? 'has-error' : NULL}} xs-pt-10">
                                    <label class="control-label">Teacher</label> 
                                    {{Form::select('adviser_id',$teachers,old('adviser_id',$sections->adviser_id),['class' => "form-control select input-sm"])}}
                                    @if($errors->first('adviser_id'))
                                    <span class="help-block">{!!$errors->first('adviser_id')!!}</span>
                                    @endif
                                </div>
